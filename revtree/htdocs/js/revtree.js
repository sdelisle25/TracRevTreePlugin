(function() {
  ...
  var scroll_src = function(event) {
      ...
      $('html,body').animate({scrollLeft: widget.offset().left - xoffset + (widget.width() / 2),
                              scrollTop: widget.offset().top - yoffset - (widget.height() / 2)}, 500);
      event.preventDefault();
      event.stopPropagation();
  }
  window.scroll_src = scroll_src;

  var scroll_dest = function(event) {
    var widget = $("[changeset='" + window.dest_rev + "']");
    var xoffset;
    var yoffset;
  ...
  window.scroll_dest = scroll_dest;
    ...
    $(document).bind('keydown', function(event) {
      ...
      if (event.ctrlKey || event.metaKey) {
        switch(String.fromCharCode(event.which).toLowerCase()) {
          ...
          case 's':
            window.scroll_src(event);
          break;

          /* Goto destination changeset */
          case 'd':
            window.scroll_dest(event);
          break;
        }
      }
      else {
  ...
  function revtree_mouseover(widget, revision)
  {
    ...
    /* Tool tip tittle */
    div = $("#tooltip_title");
    div.hover(function() { window.tooltip_cancel(); },
              function() { window.revtree_mouseout();});
    ...
    $.ajax({async: true, type: "GET", dataType: 'text',
            url: logurl, success: loginfo_update});
    ...
    function revtree_mouseout(event) {
      window.tooltip_cancel();
      window.tooltip_timer = setTimeout("tooltip_remove();", 300);
    };

    window.tooltip_cancel = function() {
    ...
    window.tooltip_remove = function() {
      var div = $("#tooltip");
  ...
  /* RevTree object */
  var RevTree = function(tree, url, style)
  {
  ...
  window.abort_action_selector = function(event) {
      $("#svg").hide();
  ...
  RevTree.prototype.render = function(scale)
  {
    ...
    renderer.writeAttributeString("xmlns:svg", "http://www.w3.org/2000/svg");
    ...
    renderer.writeAttributeString("font-size", $("#metrics").css("font-size"));
    renderer.writeAttributeString("font-family", $("#metrics").css("font-family"));
    renderer.writeAttributeString("transform", "scale(0.6)");

    for(var idx=0; idx < layers.length; idx++) {
        for (var brc=0; brc < this._branches.length; brc++) {
          renderer.writeStartElement('svg:g');
          renderer.writeAttributeString("group", this._branches[brc]._group);
    ...
    var xmlDoc = (new DOMParser()).parseFromString(renderer.flush(),'text/xml')
